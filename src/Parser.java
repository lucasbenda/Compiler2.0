...
public class Parser {
    ...
    LinkedList<Tokenizer.Token> tokenList = new LinkedList<>();
    private Tokenizer.Token scanToken;
    int index = -1;

    //private final SymbolTable table;
    public Parser(LinkedList<Tokenizer.Token> inList) {
    ...
    private Token matchTokens(Tokenizer.Token in) {
        Token out = new Token();
        Type match;
        match = Type.valueOf(in.getToken());
        out.type = match;
        out.lexeme = in.getLexeme();
        out.row = in.getLineNumber();
        out.colomn = in.getColumnNumber();
    ...
    private void prime() {
        ...
        Tokenizer.Token next = tokenList.get(index);
        lookaheadToken = matchTokens(next);
...
////////////////////////////
    private void systemGoal() {
        // rule 1
        program();
        if (lookahead == Type.MP_EOF) {
            System.out.println("The file has finished Parsing");
        } else {
            error("EOF was not found");
        }
